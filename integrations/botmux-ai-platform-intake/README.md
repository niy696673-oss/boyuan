...
- The downloaded BP copy is deleted as soon as the platform upload request succeeds or fails, before Luna analysis and card delivery. Cleanup is retried and a cleanup failure keeps the receipt recoverable instead of silently leaking the file. Once a complete job is durable, the redundant processing-card receipt is deleted; retryable pre-upload failures are retained for restart recovery, while permanently unsupported files are marked terminal and skipped at startup.
