...
Conversation state is isolated by chat and sender and persisted alongside `statePath` in `.conversations.json`. Each private conversation reuses one OpenCode session ID, checkpointed before its first prompt, including after receiver restarts. Text and BP messages share a durable FIFO inbox: the next message waits for analysis and delivery, including bounded delivery retries (three attempts), while different conversations proceed independently. A terminal failure releases the queue and is reported rather than blocking it forever. Restart recovery reuses stored decisions, cards and completed members; duplicate event delivery does not create another card.

The last six turns plus up to three recent completed BP records provide explicit follow-up context. BP context contains parsed original text with available locators and the result card, marked as unverified material claims. The receiver can recover previously uploaded BPs only after the internal API checks channel, original message, file key and sender against the persisted receipt. No new public workbench route is exposed. Source text is capped at 60,000 characters per document (head and tail retained), with an explicit omission marker; repeated material context is bounded to 80,000 characters. Do not claim unseen text was read. Inputs over 16,000 characters receive a request to split the message rather than being silently truncated; one research request can contain up to 20 companies with two active company jobs at a time. These are capacity bounds, not a command grammar.
