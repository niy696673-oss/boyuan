...
function researchPrompt(
  ...
  return [
    ...
    '请给出简洁研究摘要、可确认候选和结构化公司关系。每条候选和每条关系都必须引用本次 webResults 中至少一个 URL，并且 evidenceUrls 必须逐字复制 webResults 提供的原样完整 URL；不得改写、补全、缩短或自行生成 URL。没有可靠公开证据时，对应数组返回空数组。',
    'relations 的 category 只能是 upstream、downstream、customer、competitor：供应商、原材料、设备或技术提供方归 upstream；渠道、交付或其他下游伙伴归 downstream；明确客户归 customer；直接竞品、替代方案或潜在竞对归 competitor。targetName、relationType 和 description 只能陈述所引用公开来源直接支持的内容，不得用行业常识补全，也不得把当前公司自身作为 targetName。',
    `输出 schema：${JSON.stringify({ summary: '研究摘要', candidates: [{ knowledgeType: 'external_update', statement: '完整陈述', value: '可选', effectiveAt: '可选', evidenceUrls: ['https://example.com/source'], highImpact: false, sensitive: false }], relations: [{ targetName: '公开来源明确提及的关联公司或机构', category: 'upstream', relationType: '供应商或技术提供方等具体关系', description: '公开来源直接支持的关系事实', evidenceUrls: ['https://example.com/source'] }] })}`,
