...
- `relationships/`：关联性全景的统一读取与写入边界；关系类型固定为 upstream/downstream/customer/competitor，来源固定为 bp_self_report/project_library/external。
...
关联性全景只在同一来源内按“关系类别 + 目标公司 ID（没有 ID 时用规范化名称）+ 具体关系标签”去重，并合并证据；跨来源相同关系始终分别保留。展示优先级为：已确认企业项目库、冲突/待确认企业项目库、外部来源、BP 自陈。BP 自陈与外部来源固定为未核验；企业项目库沿用正式公司关系状态。外部关系只能引用本次 `webResults` 中逐字匹配的完整 HTTP(S) URL，校验后与研究结果在同一事务中持久化。
