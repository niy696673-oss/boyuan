...
export function createOpenCodeAnalysisAdapter(options: OpenCodeAnalysisOptions): MaterialAnalysisPort {
  ...
  return {
    async analyze(input): Promise<MaterialAnalysisResult> {
      ...
      const skillLoaded = !options.skillName || successfulToolParts.some((part) => (
        part.tool === 'skill' && part.state?.input?.name === options.skillName
      ));
      const sequentialThinkingUsed = !options.sequentialThinkingTool
        || successfulToolParts.some((part) => part.tool === options.sequentialThinkingTool);
      const missingTool = !skillLoaded ? `skill:${options.skillName}`
        : !sequentialThinkingUsed ? options.sequentialThinkingTool
          : undefined;
      if (missingTool) {
...
function analysisPrompt(input: MaterialAnalysisInput, skillName?: string, sequentialThinkingTool?: string): string {
  ...
  return [
    ...
    ...(skillName ? [`先调用 skill 工具加载“${skillName}”，并严格遵循其中的分析流程与证据边界。`] : []),
    ...(sequentialThinkingTool ? [`在生成最终 JSON 前调用“${sequentialThinkingTool}”工具检查主体识别、13 维覆盖、证据引用和冲突。`] : []),
